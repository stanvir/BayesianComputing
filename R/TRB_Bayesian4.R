...
#Raw Data Import
Traffic<- read.csv("input_validation_formatted.csv")
...
Y<-Traffic[,4]/Traffic[,14]  # volume normalized over lanes
Y <- as.integer(Y)
...
params <- c("alpha","alpha_segment","beta2","beta22","beta3",
            "beta5","beta_Flag","beta6")
...
## e.g. to get the mean and SD of prediction


meanpred<-as.data.frame(summary(samp)[[1]][,1])

#plot this mean predicted value against the observed data
...
#get all the posterior distribution
samp_all <- coda.samples(model, n.iter=10000,
                     variable.names= params)
#get the mean of the variables
mean_var<-summary(samp_all)[[1]][,1]
mean_var_mat<-as.matrix(mean_var)


#read the test dataset. 
dat<-read.csv("input _3months.csv",header=T)

#prepare the test dataset to multiply with the variable matrix
dat$seg1<-ifelse(dat$Segment=="B-NQ",1,0)
dat$seg2<-ifelse(dat$Segment=="B-Q",1,0)
dat$seg3<-ifelse(dat$Segment=="W-NQ",1,0)
dat$seg4<-ifelse(dat$Segment=="W-Q",1,0)

dat$trate<-dat$avgTravelrate/max(dat$avgTravelrate)
dat$trate2<-dat$trate^2
dat$trate3<-dat$trate^3
dat$trateseg1<-ifelse(dat$Segment=="B-NQ",dat$trate,0)
dat$trateseg2<-ifelse(dat$Segment=="B-Q",dat$trate,0)
dat$trateseg3<-ifelse(dat$Segment=="W-NQ",dat$trate,0)
dat$trateseg4<-ifelse(dat$Segment=="W-Q",dat$trate,0)

dat$trate2seg1<-ifelse(dat$Segment=="B-NQ",dat$trate2,0)
dat$trate2seg2<-ifelse(dat$Segment=="B-Q",dat$trate2,0)
dat$trate2seg3<-ifelse(dat$Segment=="W-NQ",dat$trate2,0)
dat$trate2seg4<-ifelse(dat$Segment=="W-Q",dat$trate2,0)

dat$trate3seg1<-ifelse(dat$Segment=="B-NQ",dat$trate3,0)
dat$trate3seg2<-ifelse(dat$Segment=="B-Q",dat$trate3,0)
dat$trate3seg3<-ifelse(dat$Segment=="W-NQ",dat$trate3,0)
dat$trate3seg4<-ifelse(dat$Segment=="W-Q",dat$trate3,0)

dat$trate_t_1<-dat$TravelRate.at.t.1/max(dat$TravelRate.at.t.1)

dat$navg.adj<-dat$Avg.adj/max(dat$Avg.adj)

dat$nTime<-dat$hour/max(dat$hour)
dat$nTime2<-dat$nTime^2
dat$nTime3<-dat$nTime^3
dat$nTime4<-dat$nTime^4
dat$nTime5<-dat$nTime^5


dat$flag<-ifelse(dat$hour %in% c(7:10,16:19),1,0)

test<-cbind(1,dat$seg1,dat$seg2,dat$seg3,dat$seg4,dat$trate,dat$trate2,dat$trate3,
            dat$trateseg1,dat$trateseg2,dat$trateseg3,dat$trateseg4,
            dat$trate2seg1,dat$trate2seg2,dat$trate2seg3,dat$trate2seg4,
            dat$trate3seg1,dat$trate3seg2,dat$trate3seg3,dat$trate3seg4,
            dat$trate_t_1,dat$navg.adj,dat$nTime,dat$nTime2,dat$nTime3,dat$nTime4,dat$nTime5,
            dat$flag)
lambda_hat<-test%*%mean_var_mat



write.csv(lambda_hat,"output/lambda_hat.csv")
write.csv(mean_var,"output/variable mean train.csv")
